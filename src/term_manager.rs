...
impl TermManager {
    ...
    pub fn mk_nullable_lift(&self, kind: Cvc5Kind, args: &[Term]) -> Term {
        ...
        Term::from_raw(unsafe { cvc5_mk_nullable_lift(self.ptr_mut(), kind, raw.len(), raw.as_ptr()) })
    }
    ...
    pub fn mk_dt_cons_decl(&self, name: &str) -> DatatypeConstructorDecl {
        let c = CString::new(name).unwrap();
        DatatypeConstructorDecl::from_raw(unsafe { cvc5_mk_dt_cons_decl(self.ptr_mut(), c.as_ptr()) })
    }
    ...
    pub fn mk_dt_decl_with_params(
        ...
        DatatypeDecl::from_raw(unsafe {
            cvc5_mk_dt_decl_with_params(self.ptr_mut(), c.as_ptr(), raw.len(), raw.as_ptr(), is_codt)
        })
    }
