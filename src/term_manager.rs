...
impl TermManager {
    ...
    pub fn mk_ff_sort(&self, size: &str, base: u32) -> Sort {
        ...
        Sort::from_raw(unsafe { cvc5_mk_ff_sort(self.ptr(), c.as_ptr(), base) })
    }

    /// Create a datatype sort from a datatype declaration.
    pub fn mk_dt_sort(&self, decl: &DatatypeDecl) -> Sort {
        Sort::from_raw(unsafe { cvc5_mk_dt_sort(self.ptr(), decl.inner) })
    }
    ...
    pub fn mk_dt_sorts(&self, decls: &[DatatypeDecl]) -> Vec<Sort> {
        ...
        let ptr = unsafe { cvc5_mk_dt_sorts(self.ptr(), raw.len(), raw.as_ptr()) };
        (0..decls.len())
    ...
    pub fn mk_fun_sort(&self, domain: &[Sort], codomain: Sort) -> Sort {
        ...
        Sort::from_raw(unsafe {
            cvc5_mk_fun_sort(self.ptr(), raw.len(), raw.as_ptr(), codomain.inner)
        })
    }
    ...
    pub fn mk_param_sort(&self, symbol: &str) -> Sort {
        ...
        Sort::from_raw(unsafe { cvc5_mk_param_sort(self.ptr(), c.as_ptr()) })
    }
    ...
    pub fn mk_predicate_sort(&self, sorts: &[Sort]) -> Sort {
        ...
        Sort::from_raw(unsafe { cvc5_mk_predicate_sort(self.ptr(), raw.len(), raw.as_ptr()) })
    }
    ...
    pub fn mk_unresolved_dt_sort(&self, symbol: &str, arity: usize) -> Sort {
        let c = CString::new(symbol).unwrap();
        Sort::from_raw(unsafe { cvc5_mk_unresolved_dt_sort(self.ptr(), c.as_ptr(), arity) })
    }
    ...
    pub fn mk_uninterpreted_sort_constructor_sort(&self, arity: usize, symbol: &str) -> Sort {
        let c = CString::new(symbol).unwrap();
        Sort::from_raw(unsafe {
            cvc5_mk_uninterpreted_sort_constructor_sort(self.ptr(), arity, c.as_ptr())
        })
    }
    ...
    pub fn mk_anonymous_uninterpreted_sort_constructor_sort(&self, arity: usize) -> Sort {
        Sort::from_raw(unsafe {
            cvc5_mk_uninterpreted_sort_constructor_sort(self.ptr(), arity, std::ptr::null())
        })
    }
    ...
    pub fn mk_tuple_sort(&self, sorts: &[Sort]) -> Sort {
        let raw: Vec<Cvc5Sort> = sorts.iter().map(|s| s.inner).collect();
        Sort::from_raw(unsafe { cvc5_mk_tuple_sort(self.ptr(), raw.len(), raw.as_ptr()) })
    }

    /// Create a nullable sort wrapping the given sort.
    pub fn mk_nullable_sort(&self, sort: Sort) -> Sort {
        Sort::from_raw(unsafe { cvc5_mk_nullable_sort(self.ptr(), sort.inner) })
    }
    ...
    pub fn mk_op(&self, kind: Cvc5Kind, indices: &[u32]) -> Op {
        Op::from_raw(unsafe { cvc5_mk_op(self.ptr(), kind, indices.len(), indices.as_ptr()) })
    }
    ...
    pub fn mk_op_from_str(&self, kind: Cvc5Kind, arg: &str) -> Op {
        ...
        Op::from_raw(unsafe { cvc5_mk_op_from_str(self.ptr(), kind, c.as_ptr()) })
    }
    ...
    pub fn mk_term(&self, kind: Cvc5Kind, children: &[Term]) -> Term {
        ...
        Term::from_raw(unsafe { cvc5_mk_term(self.ptr(), kind, raw.len(), raw.as_ptr()) })
    }
    ...
    pub fn mk_term_from_op(&self, op: Op, children: &[Term]) -> Term {
        ...
        Term::from_raw(unsafe {
            cvc5_mk_term_from_op(self.ptr(), op.inner, raw.len(), raw.as_ptr())
        })
    }
    ...
    pub fn mk_nullable_lift(&self, kind: Cvc5Kind, args: &[Term]) -> Term {
        ...
        Term::from_raw(unsafe { cvc5_mk_nullable_lift(self.ptr(), kind, raw.len(), raw.as_ptr()) })
    }
    ...
    pub fn mk_skolem(&self, id: Cvc5SkolemId, indices: &[Term]) -> Term {
        ...
        Term::from_raw(unsafe { cvc5_mk_skolem(self.ptr(), id, raw.len(), raw.as_ptr()) })
    }
    ...
    pub fn mk_string(&self, s: &str, use_esc_seq: bool) -> Term {
        let c = CString::new(s).unwrap();
        Term::from_raw(unsafe { cvc5_mk_string(self.ptr(), c.as_ptr(), use_esc_seq) })
    }

    /// Create an empty sequence of the given element sort.
    pub fn mk_empty_sequence(&self, sort: Sort) -> Term {
        Term::from_raw(unsafe { cvc5_mk_empty_sequence(self.ptr(), sort.inner) })
    }

    /// Create the universe set of the given sort.
    pub fn mk_universe_set(&self, sort: Sort) -> Term {
        Term::from_raw(unsafe { cvc5_mk_universe_set(self.ptr(), sort.inner) })
    }

    /// Create a bit-vector constant of the given size and value.
    pub fn mk_bv(&self, size: u32, val: u64) -> Term {
        Term::from_raw(unsafe { cvc5_mk_bv_uint64(self.ptr(), size, val) })
    }
    ...
    pub fn mk_bv_from_str(&self, size: u32, s: &str, base: u32) -> Term {
        let c = CString::new(s).unwrap();
        Term::from_raw(unsafe { cvc5_mk_bv(self.ptr(), size, c.as_ptr(), base) })
    }
    ...
    pub fn mk_fp_from_ieee(&self, sign: Term, exp: Term, sig: Term) -> Term {
        Term::from_raw(unsafe {
            cvc5_mk_fp_from_ieee(self.ptr(), sign.inner, exp.inner, sig.inner)
        })
    }
    ...
    pub fn mk_cardinality_constraint(&self, sort: Sort, upper_bound: u32) -> Term {
        Term::from_raw(unsafe {
            cvc5_mk_cardinality_constraint(self.ptr(), sort.inner, upper_bound)
        })
    }
    ...
    pub fn mk_dt_cons_decl(&self, name: &str) -> DatatypeConstructorDecl {
        let c = CString::new(name).unwrap();
        DatatypeConstructorDecl::from_raw(unsafe { cvc5_mk_dt_cons_decl(self.ptr(), c.as_ptr()) })
    }
    ...
    pub fn mk_dt_decl(&self, name: &str, is_codt: bool) -> DatatypeDecl {
        let c = CString::new(name).unwrap();
        DatatypeDecl::from_raw(unsafe { cvc5_mk_dt_decl(self.ptr(), c.as_ptr(), is_codt) })
    }
